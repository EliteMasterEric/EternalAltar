...
public class EternalAltarTask implements Runnable {

    private final EternalAltarListener listener;
    ...
    public EternalAltarTask(EternalAltarListener listener, Block altar, int stepDelay, ItemStack output, List<Block> pedestals, List<ItemStack> items, Player player) {
        this.listener = listener;
        this.dropLocation = altar.getLocation().add(0.5, 1.3, 0.5);
        Bukkit.getLogger().info("Starting Eternal Altar Task with delay: " + stepDelay + " ticks");
    ...
    private void checkPedestal(@Nonnull Block pedestal) {
        ...
        if (!item.isPresent() || positionLock.remove(item.get()) == null) {
            ...
            // ERIC: Changed sound to be "fancier".
            pedestal.getWorld().playSound(pedestal.getLocation(), Sound.ENTITY_EVOKER_CAST_SPELL, 1F, 2F);
